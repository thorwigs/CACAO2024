	...
	public void next() {
		...
		for (Feve f : stock.keySet()) { // pas forcement equitable : on avise si on lance un contrat cadre pour tout type de feve
			if (stock.get(f).getValeur()-restantDu(f)>100) { // au moins 100 tonnes par step pendant 6 mois
				...
				double parStep = Math.max(10, (stock.get(f).getValeur()-restantDu(f))/24); // au moins 100, et pas plus que la moitie de nos possibilites divisees par 2
				Echeancier e = new Echeancier(Filiere.LA_FILIERE.getEtape()+1, 1, parStep);
