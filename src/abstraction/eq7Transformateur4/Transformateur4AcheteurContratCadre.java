...
public class Transformateur4AcheteurContratCadre extends Transformateur4AcheteurBourse implements IAcheteurContratCadre{
	...
	protected Journal journalCCachat;

	public Transformateur4AcheteurContratCadre() {
		super();
		this.contratsEnCours=new LinkedList<ExemplaireContratCadre>();
		this.contratsTermines=new LinkedList<ExemplaireContratCadre>();
		this.journalCCachat = new Journal(this.getNom()+" journal CC achat", this);
	}
	...
	public void notificationNouveauContratCadre(ExemplaireContratCadre contrat) {
		journalCCachat.ajouter("Nouveau contrat :"+contrat);
		this.contratsEnCours.add(contrat);
	}

	public void receptionner(IProduit p, double quantiteEnTonnes, ExemplaireContratCadre contrat) {
		journalCCachat.ajouter("Reception de "+quantiteEnTonnes+" T de "+p+" du contrat "+contrat.getNumero());
	...
	public void next() { 	//à modifier selon nb de fèves qu'on veut
		super.next();
		this.journalCCachat.ajouter("=== STEP "+Filiere.LA_FILIERE.getEtape()+" ====================");
				for (Feve f : stockFeves.keySet()) { // pas forcement equitable : on avise si on lance un contrat cadre pour tout type de feve
					if (stockFeves.get(f)+restantDu(f)<30000) { 
						this.journalCCachat.ajouter("   "+f+" suffisamment peu en stock/contrat pour passer un CC");
						...
						if (vendeurs.size()>0) {
							...
							journalCCachat.ajouter("   "+vendeur.getNom()+" retenu comme vendeur parmi "+vendeurs.size()+" vendeurs potentiels");
							ExemplaireContratCadre contrat = supCC.demandeAcheteur(this, vendeur, f, e, cryptogramme, false);
							if (contrat==null) {
								journalCCachat.ajouter(Color.RED, Color.white,"   echec des negociations");
							} else {
								this.contratsEnCours.add(contrat);
								journalCCachat.ajouter(Color.GREEN, vendeur.getColor(), "   contrat signe");
							}
						} else {
							journalCCachat.ajouter("   pas de vendeur");
						}
					}
				}
		...
		for (ExemplaireContratCadre c : this.contratsTermines) {
			journalCCachat.ajouter("Archivage du contrat "+c);
			this.contratsEnCours.remove(c);
		}
		this.journalCCachat.ajouter("=================================");
	}
	
	public List<Journal> getJournaux() {
		List<Journal> jx=super.getJournaux();
		jx.add(journalCCachat);
		return jx;
	}
}
